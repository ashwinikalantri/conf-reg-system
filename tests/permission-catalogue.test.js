...
(async () => {
  ...
  const ADDED_SINCE_BASELINE = {
    ...
    'POST /api/registrations/:id/fee-adjustment': "Reduces what a delegate owes, in two modes. EARLY_PHASE honours the price in force when their money actually reached the bank -- a delegate who paid inside the early-bird window and submitted the form after it closed was charged the later price and then chased for a shortfall that was an accident of paperwork; the server re-derives the figure from the linked bank credit's date, so no amount is accepted from the caller. DISCRETIONARY takes an amount and a written reason. Its own key (payments.adjust_fee) rather than payments.revise, because that one re-prices when a FACT changed (the category) and is held by the front desk -- correcting a category at a counter is desk work, writing off a fee is not. Super Admin only; no existing role gains it.",
    'POST /api/users/:phone/reset-password': "Replaces somebody's password with a one-time value read out at the counter, and requires them to choose their own at their next sign-in. Its own key (users.reset_password) rather than users.edit, because the two are different acts: users.edit changes what an account SAYS, this changes who can get INTO it. Separating them lets a front desk hand out credentials without also being able to rewrite a delegate's demography, and lets a role that edits details be withheld from handing out access. Held by Super Admin (grants_all) and Front Desk; no existing role gains it.",
    'GET /api/desk/search': "Type-ahead for the desk's search box. Separate from the lookup because it answers a different question -- the lookup RESOLVES one person from something they produced, this OFFERS candidates for a few characters. Reads names, numbers and email addresses, so it is desk.view like the lookup itself; it returns at most 8 rows and refuses a query under two characters, since one character matches a large share of a conference.",
