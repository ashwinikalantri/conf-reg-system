...
// Runs INSIDE the app container (see scripts/daily-digest.sh, which invokes
// it via `docker compose exec`), not on the host. That keeps it on the same
// Node as the app itself -- this script imports @aws-sdk/client-sesv2, which
// declares engines >=20, so running it on the host's older Node was below
// the SDK's own minimum. Inside the container, APP_DIR is /app and both
// .env and conference.db there are symlinks onto the /data volume, so these
// ordinary relative paths resolve to exactly the same files the live app
// reads and writes -- no special-casing needed.
